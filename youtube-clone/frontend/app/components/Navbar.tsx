  ...
  useEffect(() => {
    const syncUserProfile = async () => {
      ...
      try {
        ...
        setUserPlan(freshData.plan || 'Free');
        setUserName(freshData.name?.charAt(0).toUpperCase() || 'U');

        // Sync local storage for WatchPage and Profile
        localStorage.setItem('userPlan', freshData.plan);
        ...
        if (localPlan) setUserPlan(localPlan);
      }
    };
